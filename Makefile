...
SLIVERDIR := sliver-extensions/godpotato

.PHONY: all clean check scanbuild test sliver

all: clean sliver

# Build and copy .o files into the Sliver extension directory
sliver: dist/$(BOFNAME).x64.o dist/$(BOFNAME).x86.o
	cp dist/$(BOFNAME).x64.o $(SLIVERDIR)/
	cp dist/$(BOFNAME).x86.o $(SLIVERDIR)/
...
clean:
	rm -f $(BOFNAME).*.exe
	rm -rf build dist
	rm -f $(SLIVERDIR)/$(BOFNAME).x64.o $(SLIVERDIR)/$(BOFNAME).x86.o
